...
Each daily run does four things:
1. Writes the full brief to `briefs/YYYY-MM-DD.md` for the dated archive.
2. Overwrites `LATEST_BRIEF.md` at the **project root** with the same content,
   so the latest brief is always one tap from the project home screen in the
   Claude Code mobile app.
3. Creates a compact HTML email digest (TL;DR + section headlines + source
...
4. Sends a push notification to your phone with a one-line summary of the
   day's top headlines. Commits and pushes both brief files to the branch.
